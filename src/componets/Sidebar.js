...
const Sidebar = ({ photoURL, userName, userList, uid }) => {
  const suggestedUsers = () => {
    let randomUserList = [];

    for (
      let i = 0;
      i < (userList.length - 1 < 5 ? userList.length - 1 : 5);
      i++
    ) {
      randomUserList.push(
  ...
  return (
    <div className={styles.container}>
      <div className={styles.profileContainer}>
        <Link to={`/${uid}`} style={{textDecoration: "none", display: "flex", alignItems: "center", color: "#262626"}}><div className={styles.profilePhotoContainer}>
